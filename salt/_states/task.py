...
def _get_task_state_data(name,
    ...
    if _valid_location(location):
        ...
        task_state['task_found'] = name in __salt__[
            'task.list_tasks'](location)
        # if task was found then get actions and triggers info
        if task_state['task_found']:
            task_info = __salt__['task.info'](name, location)
            task_state['task_info'] = {key: task_info[key]
                                       for key in TASK_PARTS}
            ...
            if key in trigger:
                ...
                try:
                    ...
                    trigger[key] = salt.utils.dateutils.strftime(
                        "{0}-{1}-{2}".format(year, month, day), '%Y-%m-%d')
                except IndexError:
                    pass
                except ValueError:
                    pass
...
def _get_task_state_prediction(state,
    ...
    if state['location_valid']:
        ...
        argument_add_on = set(sum([TRIGGER_PARTS[key]
                              for key in TRIGGER_PARTS], []))
...
def present(name,
    ...
    if not before['location_valid']:
        ret['result'] = False
        ret['comment'] = '{0} is not a valid file location'.format(
            repr(location))
        return ret
    ...
    try:
        if int(__grains__['osversion'].split('.')[0]) >= 8 and new_task['action']['action_type'] in ['Email', 'Message']:
            log.warning('This OS %s does not support Email or Message action_type.', __grains__[
                        'osversion'])
    except ValueError:
        pass
